...
- **Clean installs.** The installed app and the development build no longer share a settings folder. The development build (`npm run dev`, `npx electron .`) keeps its recent-notebooks list, global templates, and browser storage in `PageBinder Dev`; an installed copy uses `PageBinder`. The first time an installed copy starts, it moves any settings left there by development into `PageBinder Dev`, so it opens with an empty recent list and the Getting started note; later starts and updates leave a user's settings alone. The DigiNote-era settings are carried into the development folder only. `scripts/check-package.ts`, run by both build workflows, fails the build if a package holds anything but the program, its help documents, and its libraries.
