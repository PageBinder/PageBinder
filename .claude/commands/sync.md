...
1. **Know this machine.** Read `CLAUDE.local.md`. If it is missing, ask the user whether this machine is the lead Mac or a platform machine and what platform it is, then create `CLAUDE.local.md` from `docs/dev/CLAUDE.local.example.md` (it is git-ignored; never commit it).
...
4. **If `package-lock.json` changed** since the noted commit, run `npm ci` inside `app/` (then `node node_modules/electron/install.js` there if Electron is missing).
5. **Read what happened elsewhere:** the newest entries in `docs/dev/HANDOFF.md` (at least all entries newer than the noted commit), `git log --oneline <noted commit>..HEAD`, and any changes to `docs/dev/DECISIONS.md` and `docs/dev/PLATFORM_NOTES.md` in that range.
