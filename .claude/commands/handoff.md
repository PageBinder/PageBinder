...
1. **Know this machine** from `CLAUDE.local.md` (role and platform). If it is missing, ask the user and create it from `docs/CLAUDE.local.example.md`.
2. **Verify before recording.** If code changed in this task, run `npm run typecheck` and `npm test`, plus the end-to-end suites the change touches. Record exactly what ran and the results; never claim a check that did not run.
3. **Write the note** at the top of `HANDOFF.md`, below the `---` line, in the entry format that file shows. Title it with today's date, this machine's role and platform, and the branch. Use $ARGUMENTS as the gist if given. Cover what changed (with the commit range), what was found, what was checked, what the next platform must check, and what is open. Keep only the 20 newest entries.
4. **Record lasting knowledge where it belongs:**
   - A choice that should outlive the session goes in `docs/DECISIONS.md`, as one line under the right heading.
   - A platform-specific lesson goes in `docs/PLATFORM_NOTES.md`.
   - A user-visible change goes in the version section of `DEVELOPMENT_PLAN.md`.
