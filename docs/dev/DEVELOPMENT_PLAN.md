...
- **Printout text is searchable.** When an attached PDF is inserted as a printout, each page's text layer is kept with its picture (`ImageObject.printout`: source name, page number, text). Search lists those pages under **In printouts**; opening one counts the printout's matches with the page's own and outlines the picture holding the current one. `page.html` lays the text transparently over each printout picture, so a browser's find-on-page and PDFs exported from it can find it. Attachments that are not printed out never contribute their contents, only their names. The search index moved to version 4 and rebuilds itself on first open. Scanned PDFs have no text layer and stay unsearchable; see `FUTURE_FEATURES.md`.
- **Text across a page break looks as it prints.** When page.html is printed, its pagination script pushes any paragraph, list item, or table that would straddle a sheet boundary down to the next sheet's printable area. The editor now applies the same rule live (`app/src/renderer/src/sheetBreaks.ts`), as ProseMirror node decorations that add top margin, so the page itself never changes and undo never sees it. It recalculates after every edit, move, resize, paper change, or zoom. The selector and rule mirror `paginate.ts` and must stay in step with it; the phase 8 suite compares the pushes made in the editor with those made in page.html.
- **Stacking order.** Every object's right-click menu has Order > Bring to front, Bring forward, Send backward, and Send to back, for one object or a selection. Objects stack in the order the page stores them, in the editor and in page.html alike, so the commands reorder that list, and undo reverses them.
